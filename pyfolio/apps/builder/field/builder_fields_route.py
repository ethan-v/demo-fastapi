...
@router.post("/{table}/create-from-sample", status_code=status.HTTP_201_CREATED)
def create_fields_from_sample(table: str, data: dict, db: Session = Depends(get_db)):
    created_fields = BuilderFieldRepository(db).create_fields_from_sample(schema_name=table, data=data)
    if created_fields is None:
        raise HTTPException(status_code=500, detail="Failed to create schema fields from sample")
    else:
        return SuccessResponse(
            message='Created schema fields from sample',
            data=created_fields,
        )
